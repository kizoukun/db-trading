...
@section("content")
    <div class="flex justify-between">
        ...
        <div>
            <a href="{{ url('/admin/bank-list/create') }}" class="bg-blue-500 hover:bg-blue-600 text-white p-3 rounded-lg ">CREATE</a>
        </div>
    </div>
            ...
            @foreach($bankLists as $bank)
                <tr>
                    ...
                    <td class="p-3 text-center space-y-2    ">
                        <a href="{{ url('/admin/bank-list/' . $bank->code) }}" class="inline-flex justify-center items-center space-x-2 border font-semibold focus:outline-none px-2 py-1 leading-5 text-sm rounded border-gray-300 bg-white text-gray-800 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none">
                        ...
                        <form id="deleteForm" method="POST" action="{{ url('/admin/bank-list/' . $bank->code) }}">
                            @csrf
                            @method("DELETE")
