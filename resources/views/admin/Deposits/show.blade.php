...
@section("content")
    <div class="flex justify-between">
        <h1 class="text-black font-bold text-4xl">Admin Deposits</h1>
    </div>
            ...
            @foreach($deposits as $deposit)
                <tr>
                    ...
                    <td class="p-3 text-center space-y-2    ">
                        <a href="{{ url('/admin/deposits/' . $deposit->id) }}" class="inline-flex justify-center items-center space-x-2 border font-semibold focus:outline-none px-2 py-1 leading-5 text-sm rounded border-gray-300 bg-white text-gray-800 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none">
                        ...
                        <form id="deleteForm" method="POST" action="{{ url('/admin/deposits/' . $deposit->id) }}">
                            @csrf
                            @method("DELETE")
